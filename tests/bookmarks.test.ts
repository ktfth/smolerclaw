...
describe("Bookmarks — CRUD", () => {
	...
	test("saveBookmark extracts domain", () => {
		const bk = saveBookmark(
			"https://www.github.com/ktfth/smolerclaw",
			"Smolerclaw",
		);
	...
	test("saveBookmark with tags", () => {
		const bk = saveBookmark("https://docs.bun.sh", "Bun Docs", {
			tags: ["bun", "runtime"],
		});
	...
	test("updateBookmark updates updatedAt timestamp", async () => {
		const bk = saveBookmark("https://example.com", "Test");
...
describe("Bookmarks — Search", () => {
	beforeEach(() => {
		...
		mkdirSync(TEST_DIR, { recursive: true });
		initBookmarks(TEST_DIR);
		saveBookmark("https://docs.bun.sh", "Bun Docs", {
			tags: ["bun", "runtime"],
			description: "Official Bun documentation",
		});
		saveBookmark("https://github.com/ktfth/smolerclaw", "Smolerclaw Repo", {
			tags: ["project", "ai"],
		});
		saveBookmark("https://developer.mozilla.org", "MDN Web Docs", {
			tags: ["web", "reference"],
			description: "Web APIs reference",
		});
		saveBookmark("https://github.com/oven-sh/bun", "Bun Source", {
			tags: ["bun", "runtime"],
		});
	});
